...
export function build(ctx) {
  ...
  try {
    ...
    for (const [E_, x0, x1] of [[E2, 65.5, ex2.x], [E1, LOBBY.xMax, ex1.x - 0.4]]) { const len = Math.hypot(x1 - x0, S0 - SUB); const g = new THREE.PlaneGeometry(len + 0.6, E_.zMax - E_.zMin + 0.5); scaleUV(g, len, 4); g.rotateX(Math.PI / 2); g.rotateZ(Math.atan2(S0 - SUB, x1 - x0)); merger.add(mat.subCeiling, g, { x: (x0 + x1) / 2 - 0.6, y: (S0 + SUB) / 2 + 2.3, z: (E_.zMin + E_.zMax) / 2 }); }
  ...
  try {
    ...
    for (const E_ of [E5, E6]) { wallX(E_.xMin, E_.z0, E_.z1 + 0.4, -1, E_.y0, S0 + 0.9, mat.creamTile, 0.25, 'exitTrench'); wallX(E_.xMax, E_.z0, E_.z1 + 0.4, 1, E_.y0, S0 + 0.9, mat.creamTile, 0.25, 'exitTrench'); wallZ(E_.z1 + 0.4, E_.xMin, E_.xMax, 1, S0 - 0.3, S0 + 0.9, mat.creamTile, 0.25, 'exitTrench');
      const len = Math.hypot(E_.z1 - E_.z0, S0 - E_.y0); const g = new THREE.PlaneGeometry(E_.xMax - E_.xMin + 0.5, len + 0.5); scaleUV(g, 3, len); g.rotateX(Math.PI / 2 - Math.atan2(S0 - E_.y0, E_.z1 - E_.z0)); merger.add(mat.subCeiling, g, { x: (E_.xMin + E_.xMax) / 2, y: (S0 + E_.y0) / 2 + 2.2, z: (E_.z0 + E_.z1) / 2 + 0.6 }); }
    // signs
