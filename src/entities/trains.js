...
//   scene.add(train.group); train.placeAlong(track, s); train.setDoors(true, { side: 'right' }); …
//
...
//   setDisplay, exteriorBoxes, placeAlong, update, setSpeed, setDestination, dispose.
//
...
// through 180° in its `body` group.
//
// Draw-call budget: one merged mesh per material per car (~16 for a trailer, ~20 for a DM) plus a
// handful of InstancedMeshes per train (wheels, door leaves, LED displays, line diagrams, posters)
// → about 130–150 draw calls for a 7-car train. Lights: none — the saloon is lit by emissive panels.
// ---------------------------------------------------------------------------
import * as THREE from 'three';
import { STOCK_1996, STOCK_S7, trainLength, carOffsets } from './trainSpec.js';
import { xAt, topOf, profileStrip, profileRibbon, profileCap, boxAt, cylAt, planeAt, Collector, decalAtlas, trainMaterials, decalMaterial, wheelGeometry, leafGeometry } from './trainParts.js';
...
function buildExterior(spec, lay, col, atlas) {
  const P = spec.profile; const { half, isDM, cabZ, doorways, doorTop, isS7, len } = lay;
  ...
  for (const side of [-1, 1]) {
    // band A: below the door sill, full length (blue skirt band + body)
    if (isS7) col.add('blue', profileStrip(P, bb, sill, -half, half, { side }));
    else { col.add('blue', profileStrip(P, bb, bb + 0.24, -half, half, { side })); col.add('body', profileStrip(P, bb + 0.24, sill, -half, half, { side })); }
    ...
    for (const [z0, z1] of spansOutsideDoors(lay)) {
      ...
      for (const [pz0, pz1] of pillarSpans(z0, z1)) col.add('body', profileStrip(P, wb, wt, pz0, pz1, { side }));
      for (const [gz0, gz1] of paneSpans(z0, z1)) {
        ...
        const gk = 0.03;
        col.add('rubber', profileStrip(P, wb - gk, wb + 0.004, gz0 - gk, gz1 + gk, { side, inset: 0.004 }));
        col.add('rubber', profileStrip(P, wt - 0.004, wt + gk, gz0 - gk, gz1 + gk, { side, inset: 0.004 }));
        col.add('rubber', profileStrip(P, wb, wt, gz0 - gk, gz0 + 0.004, { side, inset: 0.004 }));
        col.add('rubber', profileStrip(P, wb, wt, gz1 - 0.004, gz1 + gk, { side, inset: 0.004 }));
      }
    }
    // doorway jambs (thin black rubber edges) + the head strip + a stainless sill plate outside
    for (const d of doorways) {
      for (const edge of [d.zMin, d.zMax]) col.add('rubber', profileStrip(P, sill, doorTop, edge - 0.012, edge + 0.012, { side, inset: 0.002 }));
      col.add('rubber', profileStrip(P, doorTop, doorTop + 0.025, d.zMin - 0.012, d.zMax + 0.012, { side, inset: 0.002 }));
      col.add('steel', profileStrip(P, sill - 0.03, sill + 0.005, d.zMin - 0.012, d.zMax + 0.012, { side, inset: -0.004 }));
      // S7: orange door indicator lamp above each doorway
      if (isS7) col.add('indicator', boxAt(0.05, 0.05, 0.16, side * (xAt(P, doorTop + 0.09) + 0.01), doorTop + 0.09, d.z));
    }
    // band E: cant rail (body) then the roof
    col.add('body', profileStrip(P, doorTop, cantRail, -half, half, { side, steps: 2 }));
    col.add('roof', profileStrip(P, cantRail, top, -half, half, { side, steps: 6 }));
    // rain strip / cant-rail moulding
    col.add('rubber', profileStrip(P, cantRail - 0.02, cantRail + 0.02, -half + 0.05, half - 0.05, { side, inset: -0.01, steps: 1 }));
    // cab sides (DM): cab door with window, driver's window
    if (isDM) {
      const dz0 = -half + 0.22, dz1 = -half + 0.90; const wz0 = -half + 1.02, wz1 = cabZ - 0.14;
      col.add(isS7 ? 'body' : 'blue', profileStrip(P, sill, doorTop, dz0, dz1, { side, inset: 0.018 }));
      col.add('rubber', profileStrip(P, sill, doorTop, dz0 - 0.012, dz0, { side })); col.add('rubber', profileStrip(P, sill, doorTop, dz1, dz1 + 0.012, { side }));
      col.add('rubber', profileStrip(P, doorTop, doorTop + 0.012, dz0 - 0.012, dz1 + 0.012, { side }));
      col.add('glass', profileStrip(P, wb + 0.05, wt - 0.06, dz0 + 0.09, dz1 - 0.09, { side, inset: 0.03 }));
      col.add('rubber', profileStrip(P, wb + 0.02, wb + 0.05, dz0 + 0.06, dz1 - 0.06, { side, inset: 0.016 })); col.add('rubber', profileStrip(P, wt - 0.06, wt - 0.03, dz0 + 0.06, dz1 - 0.06, { side, inset: 0.016 }));
      ...
      col.add('rubber', profileStrip(P, wb - 0.03, wb, wz0 - 0.03, wz1 + 0.03, { side, inset: 0.004 })); col.add('rubber', profileStrip(P, wt, wt + 0.03, wz0 - 0.03, wz1 + 0.03, { side, inset: 0.004 }));
      col.add('body', profileStrip(P, sill, doorTop, -half, dz0 - 0.012, { side })); col.add('body', profileStrip(P, sill, doorTop, dz1 + 0.012, wz0 - 0.12, { side })); col.add('body', profileStrip(P, sill, doorTop, wz1 + 0.12, cabZ, { side }));
      col.add('body', profileStrip(P, wb, wt, wz1 + 0.12, cabZ, { side }));
    }
    // decals: roundel + line name near one end, unit number near both ends (on band B)
    if (atlas) {
      const yB = (sill + wb) / 2 + 0.02; const zr = isDM ? cabZ + 0.45 : -half + 0.55;
      col.add('decal', decalOn(atlas.rect('roundel'), 0.30, 0.30, exteriorMatrix(P, yB, zr, side)));
      col.add('decal', decalOn(atlas.rect('lineName'), 0.56, 0.28, exteriorMatrix(P, yB, zr + 0.50, side)));
      for (const zn of [half - 0.55, isDM ? -half + 1.0 : -half + 0.55 + 1.05]) col.add('decal', decalOn(atlas.rect('unitNo'), 0.30, 0.30, exteriorMatrix(P, sill + 0.24, zn, side)));
    }
    // inter-car barrier (1996 closed ends): black rubber panel bridging half the gap at the corner
    if (!isS7) for (const [zEnd, dir] of isDM ? [[half, 1]] : [[-half, -1], [half, 1]]) col.add('rubber', boxAt(0.025, 1.7, spec.gap / 2 + 0.02, side * (xAt(P, 1.5) - 0.10), 1.55, zEnd + dir * spec.gap / 4));
    // solebar / step strip along the bottom edge
    col.add('dark', boxAt(0.05, 0.14, len - 0.3, side * (xAt(P, bb) - 0.04), bb - 0.06, 0));
  }
  // underside closure, equipment cases, couplers
  col.add('dark', boxAt(2 * xAt(P, bb) - 0.06, 0.05, len - 0.05, 0, bb - 0.02, 0));
  const zb = len * spec.bogieSpacing / 2;
  ...
  for (const [zEnd, dir] of [[-half, -1], [half, 1]]) {
    if (isDM && dir < 0) { col.add('dark', boxAt(0.36, 0.28, 0.5, 0, 0.72, -half - 0.22)); col.add('dark', cylAt(0.06, 0.6, 0, 0.72, -half - 0.25, { axis: 'z', seg: 8 })); }
    else { col.add('dark', cylAt(0.07, spec.gap / 2 + 0.3, 0, 0.72, zEnd + dir * (spec.gap / 4 - 0.1), { axis: 'z', seg: 8 })); col.add('dark', boxAt(0.5, 0.3, 0.35, 0, 0.72, zEnd + dir * 0.02)); }
  }
  ...
  // roof equipment
  if (isS7) {
    for (const z of [-half + 3.2, half - 3.2]) { col.add('roof', boxAt(2.0, 0.26, 1.7, 0, top + 0.08, z)); for (const x of [-1, 1]) col.add('dark', boxAt(0.02, 0.16, 1.5, x * 1.01, top + 0.09, z)); col.add('dark', boxAt(1.7, 0.02, 0.4, 0, top + 0.215, z + 0.5)); }
    col.add('roof', boxAt(1.2, 0.12, Math.max(1, len - 9), 0, top + 0.02, 0));
  } else {
    for (const z of [-half + 2.2, 0, half - 2.2]) col.add('dark', boxAt(0.55, 0.11, 1.1, 0.30, top - 0.03, z));
    col.add('roof', boxAt(0.9, 0.05, Math.max(1, len - 3), 0, top + 0.01, 0));
  }
  // ---- car ends
  for (const [zEnd, dir] of [[-half, -1], [half, 1]]) {
    ...
    if (isS7) {
      ...
      // inter-car end door outline + window
      col.add('rubber', boxAt(0.70, 0.02, 0.02, 0, spec.floorHeight + 1.93, zEnd + dir * 0.01)); for (const x of [-0.34, 0.34]) col.add('rubber', boxAt(0.02, 1.9, 0.02, x, spec.floorHeight + 0.97, zEnd + dir * 0.01));
      col.add('glass', planeAt(0.30, 0.52, 0, spec.floorHeight + 1.575, zEnd + dir * 0.012, { ry: dir > 0 ? 0 : Math.PI }));
      col.add('rubber', planeAt(0.36, 0.58, 0, spec.floorHeight + 1.575, zEnd + dir * 0.006, { ry: dir > 0 ? 0 : Math.PI }));
    }
  }
...
/** Cab front of a DM at z = -half: red front, windows, M-door, destination display housing, lamp bezels, wipers, coupler. */
function buildCabFront(spec, lay, col, atlas) {
  const P = spec.profile; const { half, isS7 } = lay; const z = -half; const bb = spec.bodyBottom;
  const F = spec.livery.cabFront === spec.livery.doors ? 'red' : 'body';
  col.add('red', profileCap(P, z, bb, { dir: -1 }));
  const y0 = isS7 ? 1.95 : 1.62, y1 = isS7 ? 2.95 : 2.36;   // cab window band
  // black window band (S7) / window surrounds
  if (isS7) col.add('rubber', planeAt(2.62, y1 - y0 + 0.18, 0, (y0 + y1) / 2, z - 0.006, { ry: Math.PI }));
  const win = isS7 ? [[-1.2, -0.36], [0.36, 1.2]] : [[-1.06, -0.36], [0.36, 1.06]];
  for (const [x0, x1] of win) { col.add('glass', planeAt(x1 - x0, y1 - y0, (x0 + x1) / 2, (y0 + y1) / 2, z - 0.014, { ry: Math.PI })); col.add('rubber', planeAt(x1 - x0 + 0.06, y1 - y0 + 0.06, (x0 + x1) / 2, (y0 + y1) / 2, z - 0.009, { ry: Math.PI })); }
  // M door (centre) with its window, outline, handle
  ...
  col.add('rubber', boxAt(0.66, 0.02, 0.03, 0, dTop + 0.01, z - 0.015)); for (const x of [-0.32, 0.32]) col.add('rubber', boxAt(0.02, dTop - spec.doorSill + 0.02, 0.03, x, (dTop + spec.doorSill) / 2, z - 0.015));
  col.add('glass', planeAt(0.42, y1 - y0 - 0.1, 0, (y0 + y1) / 2, z - 0.026, { ry: Math.PI })); col.add('rubber', planeAt(0.48, y1 - y0 - 0.04, 0, (y0 + y1) / 2, z - 0.021, { ry: Math.PI }));
  col.add('steel', boxAt(0.03, 0.22, 0.04, 0.24, spec.floorHeight + 1.0, z - 0.03));
  // destination display housing (screen mesh added per train)
  const dy = isS7 ? 3.18 : 2.56, dw = isS7 ? 1.5 : 1.16, dh = isS7 ? 0.24 : 0.22;
  col.add('rubber', boxAt(dw + 0.08, dh + 0.06, 0.05, 0, dy, z - 0.02));
  // lamp clusters: bezels (lamps are separate switchable meshes)
  ...
  // blue lower band on the front (S7) / skirt
  if (isS7) col.add('blue', planeAt(2 * xAt(P, 0.8) - 0.1, 0.5, 0, 0.72, z - 0.006, { ry: Math.PI }));
  else col.add('blue', planeAt(2 * xAt(P, 0.5) - 0.05, 0.24, 0, bb + 0.12, z - 0.006, { ry: Math.PI }));
  // front roundel (small) on the M door and a cab-front unit number
  if (atlas) { col.add('decal', planeAt(0.22, 0.22, 0, spec.doorSill + 0.55, z - 0.026, { ry: Math.PI, uvRect: atlas.rect('roundel') })); col.add('decal', planeAt(0.26, 0.26, 0.80, spec.doorSill + 0.45, z - 0.012, { ry: Math.PI, uvRect: atlas.rect('unitNo') })); }
  // anti-climber / buffer beam and the cab roof "brow"
  col.add('dark', boxAt(2 * xAt(P, bb) - 0.2, 0.12, 0.10, 0, bb + 0.06, z - 0.05));
  col.add(F === 'red' ? 'red' : 'roof', boxAt(dw + 0.5, 0.06, 0.12, 0, dy + dh / 2 + 0.06, z - 0.05));
...
/** Everything for one car type: merged geometries per material + layout + collision model. Cached per stock/type. */
function carAssembly(spec, carIndex, atlas) {
  const isDM = carIndex === 0 || carIndex === spec.cars - 1;
  const key = `${spec.code}:${isDM ? 'DM' : 'M' + (spec.gangway && (carIndex === 1 || carIndex === 5) ? 'W' : '')}:${atlas.texture.uuid}`;
...
export class Train {
  constructor(ctx, { stock = '1996', track = null, direction = 'eastbound', destination = null, line = null, unitNumber = null } = {}) {
    ...
    this._s = null; this._near = true; this._frameCounter = 0; this._lastSpeed = 0; this._seated = null; this._playerCar = -1;
    ...
    this.setDisplay(this.line === 'jubilee' ? 'Jubilee line' : 'District line');
    this.setLights(true);
  }
  ...
  _build() {
    ...
    const atlas = this.atlas = decalAtlas(spec, { unitNumber: this.unitNumber, lineName: this.line === 'circle' ? 'Circle line' : (spec.line === 'jubilee' ? 'Jubilee line' : 'District line'), lineColor: this.line === 'circle' ? '#ffd300' : (spec.line === 'jubilee' ? '#a0a5a9' : '#00782a') });
    const mats = this.mats = trainMaterials(spec); const decalMat = decalMaterial(atlas);
    const offsets = carOffsets(spec);
    const castKeys = new Set(['body', 'roof', 'blue', 'red', 'dark']);
    // per-train dynamic textures
    this.led = T.ledStrip({ width: 1024, height: 96, color: spec.code === 'S7' ? '#ff8c1a' : '#ff9a1e' });
    ...
    const diagramTex = T.lineDiagram({ line: this.line === 'circle' ? 'Circle' : (spec.line === 'jubilee' ? 'Jubilee' : 'District'), color: this.line === 'circle' ? '#ffd300' : (spec.line === 'jubilee' ? '#a0a5a9' : '#00782a'), stations: this.line === 'circle' && spec.circleDiagram ? spec.circleDiagram : spec.lineDiagram, current: 'Westminster', width: 4096, height: 256 });
    ...
    // instanced collections: door leaves keyed by width+lead, wheels, displays, diagrams, posters
    ...
    for (const c of offsets) {
      ...
      const car = { index: c.car, offset: c.offset, len: c.length, half: c.length / 2, group: carG, body, lay, coll: asm.coll, flip: lay.flip, mat: new THREE.Matrix4(), inv: new THREE.Matrix4(), phase: Math.random() * 6.28, worldMat: new THREE.Matrix4() };
      ...
      for (const d of lay.doorways) for (const gside of [-1, 1]) {
        // lead = +1 → leading edge at +z. The leaf in the -z half of a doorway leads towards the centre (+z); a single leaf leads towards the car centre.
      ...
      for (const d of lay.displays) displaySlots.push({ car, m: surfaceMatrix(spec.interior, lay.doorTop + 0.10, d.z, d.side, -0.012), w: d.w });
      let pk = c.car % 3;
    ...
    for (const lg of leafGroups.values()) {
      const geo = leafGeometry(spec, lg.width, lg.lead);
      const im = mk(geo, [mats.red, mats.glass, mats.rubber, mats.yellow], lg.items, 'leaves'); im.castShadow = true; lg.mesh = im;
    ...
    if (displaySlots.length) mk(new THREE.PlaneGeometry(displaySlots[0].w, 0.11), this.ledMat, displaySlots, 'led');
  ...
  _buildAudio() {
    ...
    if (!audio.synths.has('train:saloon')) audio.registerSynth('train:saloon', (c, { stock = '1996' } = {}) => {
      // in-car ambience: ventilation / air-conditioning fan (broadband) + inverter hum
      ...
      const o = c.createOscillator(); o.type = 'sine'; o.frequency.value = stock === 'S7' ? 100 : 120; const og = c.createGain(); og.gain.value = 0.08;
  ...
  placeAlong(track, s) {
    ...
    // remember the car the player rides in so we can carry them with it
  ...
  _carryApply(player) {
    ...
    const dy = yawOf(car.mat) - this._carryYaw; if (Math.abs(dy) < 1) player.localYaw += dy;
  }
  ...
  interiorContains(p) {
    ...
    if (Math.abs(g.z) > car.half + spec.gap / 2 + 0.05) return false;
  ...
  resolveInterior(p, r = 0.33, height = 1.78, stepUp = 0.42) {
    ...
    if (ax > halfInt - r) {
      ...
      if (open) {
        // keep within the doorway span while in the reveal; leave the train once well outside the body
        g.z = Math.min(door.zMax - r + 0.06, Math.max(door.zMin + r - 0.06, g.z));
        if (ax > bodyHalf + 0.42) exited = true;
      } else g.x = gside * (halfInt - r);
    }
    // seats / screens (axis-aligned boxes in geometry space)
    if (Math.abs(g.x) < halfInt) for (const b of coll.boxes) {
      ...
      // never push towards the bodyside (there is a wall there)
    ...
    if (!spec.gangway) { const lim = car.half - 0.14 - r; if (g.z > lim) g.z = lim; if (!lay.isDM && g.z < -lim) g.z = -lim; }
    else {
      const gw = spec.gangway.width / 2 - r; const nearEnd = g.z > car.half - 0.5 || (!lay.isDM && g.z < -car.half + 0.5);
  ...
  exteriorBoxes() {
    ...
    for (const car of this.cars) {
      car.body.updateWorldMatrix(true, false); const wm = car.body.matrixWorld;
      const segs = []; // [zMin, zMax, xMin, xMax] in geometry space
      const openSides = [-1, 1].filter(gs => this._amountFor(car, gs) > 0.3);
      if (!openSides.length) segs.push([-car.half, car.half, -hw, hw]);
      else {
        let cursor = -car.half;
        ...
        if (car.half > cursor) segs.push([cursor, car.half, -hw, hw]);
      }
      if (spec.gangway && !(car.lay.isDM)) segs.push([-car.half - spec.gap / 2, -car.half, -0.95, 0.95]);
      if (spec.gangway) segs.push([car.half, car.half + spec.gap / 2, -0.95, 0.95]);
  ...
  _doorSounds(open, sides) {
    ...
    // doorways on the moving side(s), nearest the listener first
    const doors = [];
  ...
  setSpeed(v, accel = 0) {
    ...
    // brake release hiss when we come to a stand
    if (was > 0.8 && av < 0.05) this._airRelease();
  }
  _airRelease() {
    const audio = this.ctx.audio; if (!audio || !audio.ready) return;
  ...
  update(dt) {
    ...
    // door animation (smooth start/stop like pneumatic doors)
    ...
    // distance-based LOD for the per-frame instance updates
    ...
    // seated passenger follows the seat
  ...
  _sit(rec) {
    ...
    // face across the car (longitudinal seats) or along it (transverse)
    ...
    this._seated = { rec, seat, lastYaw: yawOf(car.mat), player, forwardG: dirG.clone() };
    if (this.ctx.audio && this.ctx.audio.ready) try { this.ctx.audio.play('footstep', { gain: 0.25, params: { surface: 'carpet' } }); } catch (e) {}
    player.yaw = yaw; player.pitch = Math.min(player.pitch || 0, 0.2);
  ...
  dispose() {
    const { ctx } = this;
    try { if (this.runEmitter) { this.runEmitter.stop(); const i = ctx.audio.emitters.indexOf(this.runEmitter); if (i >= 0) ctx.audio.emitters.splice(i, 1); } if (this.saloonEmitter) { this.saloonEmitter.stop(); const i = ctx.audio.emitters.indexOf(this.saloonEmitter); if (i >= 0) ctx.audio.emitters.splice(i, 1); } } catch (e) {}
...
// ---------------- helpers ----------------
function yawOf(m) { const e = m.elements; return Math.atan2(e[8], e[10]); }   // rotation about Y of a (rotation-only) matrix: local +z axis direction
